...
<svelte:head
    ><title>Orders — {$settings?.restaurant_name || "Pizza Mania"} Admin</title
    ></svelte:head
>
    ...
    <div class="orders-table">
        <div class="table-wrapper">
            <table>
                <thead>
                    <tr>
                        <th>Token</th><th>Type</th><th>Customer</th><th
                            >Items</th
                        ><th>Total</th><th>Payment</th><th>Status</th><th
                            >Time</th
                        ><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {#each orders as order}
                        ...
                        <tr>
                            <td style="font-weight:var(--weight-semibold)"
                                >{order.tracking_token}</td
                            >
                            <td
                                ><span class="badge badge-info"
                                    >{ORDER_TYPE_LABELS[order.order_type]}</span
                                ></td
                            >
                            <td>
                            ...
                            <td>
                                <div class="items-column">
                                    {#each expandedOrders[order.id] ? order.order_items : (order.order_items || []).slice(0, 1) as it}
                                        <div style="margin-bottom:4px">
                                            <div
                                                style="font-size:var(--text-xs);color:var(--color-text-secondary);font-weight:var(--weight-medium)"
                                            >
                                                {it.quantity}× {it.item_name}
                                            </div>
                                            {#if it.selected_options && Object.keys(it.selected_options).length > 0}
                                                <div
                                                    style="font-size:11px;color:var(--color-text-muted);padding-left:8px;border-left:2px solid var(--color-border)"
                                                >
                    ...
                    {:else}
                        <tr>
                            <td
                                colspan="9"
                                style="text-align:center;padding:var(--space-10);color:var(--color-text-muted)"
                            >
